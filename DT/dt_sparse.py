...
class TrainConfig:
    ...
    num_workers: int = 4
    # evaluation params
    target_returns: Tuple[float, ...] = (100.0, 55.0)
    eval_episodes: int = 100
    eval_every: int = 10_000
    # general params
    checkpoints_path: Optional[str] = None
    deterministic_torch: bool = False
    train_seed: int = random.randint(0, 100) 
    eval_seed: int = 42
    device: str = "cuda"
...
def load_d4rl_trajectories(
    ...
    dataset = get_sparse_dataset(dataset, env_name)
...
def eval_rollout(
    ...
    for step in range(model.episode_len):
        ...
        # Use sparse reward
        sparse_reward = 1.0 if episode_return + reward > target_return else 0.0
        returns[:, step + 1] = torch.as_tensor(returns[:, step] - sparse_reward)

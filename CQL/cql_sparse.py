#ource: https://github.com/young-geng/CQL/tree/934b0e8354ca431d6c083c4e3a29df88d4b0a24d
...
class TrainConfig:
    # Experiment
    device: str = "cuda"
    env: str = "ant-medium-v2"  # OpenAI gym environment name
    seed: int = random.randint(0, 100)  # Sets Gym, PyTorch and Numpy seeds
...
def get_sparse_dataset(dataset, env_name):
    # Ant-specific return thresholds
    ant_return_thresholds = {
        'ant-medium-v2': 3826.37,
        'ant-medium-replay-v2': 1309.06,
        'ant-expert-v2': 5252.45,
        'ant-medium-expert-v2': 5097.14
    }
    # Check if the environment is a supported Ant variant
    if env_name not in ant_return_thresholds:
        raise ValueError(f"Unsupported environment: {env_name}. This function only supports Ant environments (excluding random).")
    
    return_threshold = ant_return_thresholds[env_name]
    # Create sparse rewards
    sparse_rewards = np.zeros_like(dataset['rewards'])
    ep_ret = 0.0
    for t in range(len(sparse_rewards)):
        ep_ret += dataset['rewards'][t]
        if ep_ret > return_threshold:
            sparse_rewards[t] = 1.0
        if dataset['terminals'][t] or (dataset.get('timeouts') and dataset['timeouts'][t]):
            ep_ret = 0.0
    
    # Create a new dataset with sparse rewards
    sparse_dataset = dataset.copy()
    sparse_dataset['rewards'] = sparse_rewards  # Replace original rewards with sparse rewards
...
class ContinuousCQL:
    ...
    def _q_loss(
        ...
        if self.backup_entropy:
            target_q_values = rewards + (1.0 - dones) * self.discount * (target_q_values - alpha * next_log_pi)
        else:
            target_q_values = rewards + (1.0 - dones) * self.discount * target_q_values
...
def train(config: TrainConfig):
    ...
    # Convert to sparse rewards
    dataset = get_sparse_dataset(dataset, config.env)
